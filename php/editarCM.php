...
<?php
    ...
    function ejecuta_consulta($id)
    {
       //hacer la conexion a la base de datos
        ...
        //consulta de select
        $query1=mysqli_query($getconex, "SELECT id, id_distribuidor, marca, color, precio, stock, url_img FROM cortinas_metalicas WHERE id ='$id'");
        ...
        while ($fila=mysqli_fetch_array($query1, MYSQLI_ASSOC)) {
            echo "<script> alert ('La cortina metalica se ha encontrado correctamente'); </script>";
            
            $filas[] = $fila; // Añade el array $fila al final de $filas
        }

        }else {
            echo "<script> alert ('No se ha encontrado ninguna cortina metalica'); </script>";
        }
...
<header>
    ...
    <br>
    <p class='t2 azulP'>Buscar cortina metalica por ID</p>
    <p class='parrafo'>ID</p>
    <input type='text' placeholder='ID' name='buscar'>

    <br>
    <br>
    <input type='submit' name='buscarCM' value='Buscar cortina metalica'>
    </form>
    ...
    ?>
                ...
                <tbody>
                    ...
                    if (isset($cm) && !empty($cm)) {
                        ...
                        <tr>
                            ...
                            <td><?php echo "<form action='actualizarCM.php' method='POST' enctype='multipart/form-data'>";
                            echo "<input type='text' name='precio' value='$precio'> <br/> <br/>";
                            echo "<input type='hidden' name='id' value='$idCM'> <br/> <br/>";
                            ?></td>
                            <td><?php echo $cortinaM['stock'] ?></td>
                            <td><?php echo '<p><img src="'.$url.'"  height=200 width=250></p>';
        ...
        <?php
                        echo "<br>";
                        echo "<center><input type='submit' name='actualizarCM' value='Actualizar cortina metalica' /></center>";
                        echo "</form>"; 
                        ...
                        if (isset($cm) && !empty($cm)) {
                            ...
                            foreach ($cm as $cortinaM) {
                                ...
                                ?>
                            <tr>  
                                <td><?php echo "<form action='eliminarCM.php' method='POST' enctype='multipart/form-data'>";
                                echo "<input type='hidden' name='id' value='$idCM'> <br/> <br/>";
                                ?></td>
                            </tr>
                        ...
                        echo "<center><input type='submit' name='eliminarCM' value='Eliminar cortina metalica' /></center>";
                        echo "</form>";
                        ?>
    <?php
        } 
