...
if($_POST){
    ...
    $query = "INSERT INTO detalles_ventas (id_venta, id_producto, precio, cantidad, descargado) VALUES (?,?,?,?,?)";
    $sentencia= mysqli_prepare($getconex, $query);
    ...
    $afectado1=mysqli_stmt_affected_rows($sentencia);
      ...
      if($afectado==1){
        ...
        echo "<script> alert ('La compra se ha realizado correctamente'); location.href='CarritoCM.php'; </script>";
        unset($_SESSION['cart']);
    }

    }else{
        echo "<script> alert ('La compra no se ha realizado correctamente'); location.href='CarritoCM.php'; </script>"; 
    }
