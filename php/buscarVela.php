<?php
    ...
    function ejecuta_consulta($id)
    {
       //hacer la conexion a la base de datos
        ...
        //consulta de select
        $query1=mysqli_query($getconex, "SELECT id, id_distribuidor, marca, numero, color, precio, stock, url_img FROM velas WHERE id ='$id'");
        ...
        while ($fila=mysqli_fetch_array($query1, MYSQLI_ASSOC)) {
            echo "<script> alert ('La vela se ha encontrado correctamente'); </script>";
            
            $filas[] = $fila; // Añade el array $fila al final de $filas
        }

        }else {
            echo "<script> alert ('No se ha encontrado la vela'); </script>";
        }
...
<header>
    ...
    <br>
    <p class='t2 azulP'>Buscar vela por ID</p>
    <p class='parrafo'>ID</p>
    <input type='text' placeholder='ID' name='buscar'>

    <br>
    <br>
    <input type='submit' name='buscarVela' value='Buscar vela'>
    </form>
    ...
    ?>
                ...
                <tbody>
                    ...
                    if (isset($cm) && !empty($cm)) {
                        ...
                        <tr>
                            ...
                            <td><?php echo "<form action='actualizarVela.php' method='POST' enctype='multipart/form-data'>";
                            ...
                            <td><?php echo '<p><img src="../images_velas/'.$url.'"  height=200 width=250></p>';
        ...
        <?php
                        echo "<br>";
                        echo "<center><input type='submit' name='actualizarVela' value='Actualizar vela' /></center>";
                        echo "</form>"; 
                        ...
                        if (isset($cm) && !empty($cm)) {
                            ...
                            foreach ($cm as $cortinaM) {
                                ...
                                ?>
                            <tr>  
                                <td><?php echo "<form action='eliminarVela.php' method='POST' enctype='multipart/form-data'>";
                                echo "<input type='hidden' name='id' value='$idGL'> <br/> <br/>";
                                ?></td>
                            </tr>
                        ...
                        echo "<center><input type='submit' name='eliminarVela' value='Eliminar vela' /></center>";
                        echo "</form>";
                        ?>
    <?php
        } 
